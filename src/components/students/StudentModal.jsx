...
export default function StudentModal({ student, onClose }) {
  ...
  // Módulo de IA VIGÍA
  const { analyze, sendEmail, loading: aiLoading, error: aiError, result: aiResult, setResult: setAiResult } = useStudentAI();

  const handleAIAnalysis = async () => {
    try {
      await analyze(student);
    } catch (err) {
  ...
  const handleAISend = async () => {
    if (!aiResult) return;
    try {
      await sendEmail(student, aiResult.correo_personalizado.asunto, aiResult.correo_personalizado.cuerpo);
      actions.markIntervened(student.codigo);
      alert(`✅ Intervención de IA registrada y enviada a ${student.email}`);
    } catch (err) {
      alert("Error al enviar el correo de intervención: " + err.message);
    }
  };
    ...
    <div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          {/* AI Assistant Panel */}
          <div className="bg-slate-900/80 border border-slate-700/60 rounded-xl p-4 space-y-4 shadow-xl relative overflow-hidden">
            <div className="absolute top-0 right-0 w-24 h-24 bg-violet-600/5 rounded-full blur-xl pointer-events-none" />
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <div className="h-7 w-7 rounded-lg bg-violet-500/10 border border-violet-500/30 flex items-center justify-center">
                  <Brain size={16} className="text-violet-400 animate-pulse" />
                </div>
                <div>
                  <span className="text-sm font-bold text-slate-200 block">Asistente Copiloto IA</span>
                  <span className="text-[10px] text-slate-500">Impulsado por Gemini 2.5 Flash</span>
                </div>
              </div>
              {!aiResult && (
                <button
                  onClick={handleAIAnalysis}
                  disabled={aiLoading || student.intervenido}
                  className="px-3.5 py-1.5 rounded-lg bg-violet-600 hover:bg-violet-500 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-bold transition-all shadow-lg shadow-violet-950/40"
                >
            ...
            {aiLoading && (
              <div className="flex flex-col items-center justify-center py-6 text-xs text-slate-400 gap-2.5">
                <div className="h-5 w-5 border-2 border-transparent border-t-violet-400 rounded-full animate-spin" />
                <span>Analizando notas, asistencia y actividad en campus virtual...</span>
              </div>
            )}
            ...
            {aiResult && (
              <div className="space-y-4 animate-fade-in">
                <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/30">
                  <p className="text-xs font-bold text-violet-400 mb-1">Diagnóstico Clínico Académico:</p>
                  <p className="text-xs text-slate-300 leading-relaxed font-light">{aiResult.comentario}</p>
                </div>

                <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/30">
                  <p className="text-xs font-bold text-blue-400 mb-1.5">Correo Empático Personalizado:</p>
                  <p className="text-xs text-slate-400 font-bold mb-1">Asunto: {aiResult.correo_personalizado.asunto}</p>
                  <div className="text-xs text-slate-300 leading-relaxed font-mono whitespace-pre-wrap max-h-40 overflow-y-auto bg-slate-950/40 p-2.5 rounded border border-slate-800">
                    {aiResult.correo_personalizado.cuerpo}
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 py-2.5 rounded-lg bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold transition-all flex items-center justify-center gap-1.5 shadow-lg shadow-emerald-950/30"
                  >
                    <Send size={12} />
                    Enviar y Registrar Intervención con IA
                  </button>
                  <button
                    onClick={() => {
                      setAiResult(null);
                    ...
                    className="px-3 py-2.5 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 text-xs font-bold transition-all"
                  >
                    Regenerar
                  </button>
                </div>
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-semibold text-slate-300 flex items-center gap-2">
              <Zap size={14} className="text-violet-400" /> Plantillas de Acción Predictivas (Respaldo)
            </h3>
            {recs.map((rec, i) => (
